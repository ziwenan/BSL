...
#' We provide the data and tuning parameters required to reproduce the results in An et al. (2019).
#'
...
#' BSLasso can be found in An et al. (2019).
#'
#' \itemize{
...
#' plot(resultMa2BSL, which = 1, thetaTrue = true_ma2)
#' 
...
#' plot(resultMa2uBSL, which = 1, thetaTrue = true_ma2)
#' 
...
#'                         M = 5, sigma = 1.5, model = model, method = 'BSL', shrinkage = 'glasso')
#' sp_ma2
#' plot(sp_ma2)
#' 
...
#' plot(resultMa2BSLasso, which = 1, thetaTrue = true_ma2)
#' 
#' # Performing semiBSL (reduce the number of iterations M if desired)
#' resultMa2SemiBSL <- bsl(y = ma2$data, n = 500, M = 10, model = model, covRandWalk=ma2$cov,
#'                         method = 'semiBSL', verbose = TRUE)
#' show(resultMa2SemiBSL)
#' summary(resultMa2SemiBSL)
#' plot(resultMa2SemiBSL, which = 1, thetaTrue = true_ma2)
#' 
...
#'                 thetaTrue = true_ma2, label = c('bsl', 'uBSL', 'bslasso', 'semiBSL'),
...
#' \code{ma2_logPrior(theta)} evaluates the (unnormalised) log prior, which is uniform subject to several restrictions related to invertibility of the time series.
#' @rdname ma2
#' @export
ma2_logPrior <- function(theta) {
